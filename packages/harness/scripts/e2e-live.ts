...
async function testCoreFlow(): Promise<void> {
  ...
  try {
    ...
    assert(
      ...
      "generated mcp-config registers the hosted MCP as sapiom-direct",
    );
    assert(
      mcpConfigJson.mcpServers?.sapiom?.command === "npx",
      "generated mcp-config registers the local authoring MCP as sapiom",
    );
...
async function testAutoSessionAndMcpAuth(): Promise<void> {
  ...
  try {
    ...
    const mcpConfigJson = JSON.parse(await fs.readFile(capture.argv[mcpConfigIdx + 1], "utf8")) as {
      mcpServers?: {
        "sapiom-direct"?: { headers?: Record<string, string> };
      };
    };
    assert(
