...
### 6. The SPA's CSS is shared with the desktop onboarding window

`web/src/theme-tokens.css` (design tokens) and `web/src/primitives.css` (`.btn-primary`, `.btn-ghost`,
form resets) are copied into the Electron setup window at build time by
`harness-desktop/scripts/copy-renderer.mjs`. Renaming or dropping a token or a button class silently
breaks a window no SPA test covers.

- New token or shared component → `theme-tokens.css` / `primitives.css`.
- App-specific styling → `styles.css`.
