...
export const MOCK_HARNESSES: HarnessEntry[] = [
  {
    ...
    installed: true,
    installMcpPrompt:
      "Add the local Sapiom authoring MCP to this project: run `claude mcp add sapiom -- npx -y @sapiom/mcp`, restart the session, then run /mcp to confirm the sapiom_dev_* tools are listed.",
  ...
  },
  {
    ...
    installed: true,
    installMcpPrompt:
      "Add the local Sapiom authoring MCP to Codex: run `codex mcp add sapiom -- npx -y @sapiom/mcp`, then restart Codex and confirm the sapiom_dev_* tools are listed.",
    imageInput: true,
  },
