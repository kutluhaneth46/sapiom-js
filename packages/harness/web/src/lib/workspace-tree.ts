...
/**
 * A workspace folder: LEVEL 1 in the tree. Its label can select a dependency
 * graph while its disclosure independently folds the agents beneath it. In
 * the bare case (no agents, only live sessions), the row remains the focus
 * target so those sessions can open as tabs in the main panel.
 */
export interface WorkspaceFolder {
  ...
  isDirectory: boolean;
  /** Opaque server-issued identity for the local graph endpoint. Facet groups
   * and transient folders that were not part of the boot projection have no
   * graph identity yet. */
...
function byWorkspace(
  ...
  for (const cwd of orderedCwds) {
    ...
    const workspaceKey =
      workspaceScopes.find((scope) => samePath(scope.cwd, cwd))
        ?.workspaceKey ?? null;
    workspaces.push({
      cwd,
      label: basename(cwd),
