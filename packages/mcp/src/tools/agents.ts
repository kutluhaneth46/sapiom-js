...
export const RUN_LOCAL_TOOL_DESCRIPTION = [
  "Execute an agent on the local machine, running the author's actual step code with every ctx.sapiom.* capability call resolved from stubs. This creates no Sapiom capability request or spend, but authored JavaScript and its ordinary file, process, and network side effects still run.",
