...
- All result shapes reserve an optional `degradation` annotation (server-defined shape; absent on a clean execution).

All additions are optional/nullable: existing consumers compile and run unchanged, and results from older servers map missing fields to `null` (unknown).
